...
async function githubJson<T>(env: Env, path: string, init: RequestInit = {}): Promise<T> {
  ...
  if (!res.ok) {
    ...
    const error = new Error(`GitHub request failed: ${res.status}${detail ? ` ${detail.slice(0, 240)}` : ''}`);
...
function encodeContent(content: string): string { const bytes = new TextEncoder().encode(content); let binary = ''; bytes.forEach(byte => { binary += String.fromCharCode(byte); }); return btoa(binary); }
...
async function loadRemote(env: Env): Promise<RemoteState> {
  ...
  const indexFile = await readFile(env, 'public/data/works.json', headSha);
  ...
  const works = await Promise.all(index.works.map(async entry => {
    const file = await readFile(env, `public/${entry.data}`, headSha);
    const payload = JSON.parse(file.content) as WorkPayload;
    if (!Array.isArray(payload.items)) throw new Error(`作品資料格式無效：${entry.code}`);
    return { path: file.path, payload, sha: file.sha };
  }));
  const versionFile = await readFile(env, 'public/data/version.json', headSha);
  const version = (JSON.parse(versionFile.content) as { version?: string }).version;
  if (!version) throw new Error('version.json 格式無效');
...
async function createAtomicCommit(env: Env, remote: RemoteState, files: { path: string; content: string }[], message: string): Promise<void> {
  const treeEntries = [] as { path: string; mode: '100644'; type: 'blob'; sha: string }[];
  for (const file of files) treeEntries.push({ path: file.path, mode: '100644', type: 'blob', sha: await createBlob(env, file.content) });
  ...
  try {
    await githubJson(env, `/repos/${env.GITHUB_REPO}/git/refs/heads/${encodeURIComponent(env.GITHUB_BRANCH)}`, { method: 'PATCH', body: JSON.stringify({ sha: commit.sha, force: false }) });
  } catch (error) {
    const status = (error as Error & { status?: number }).status;
    if (status === 422) throw new Error('資料在寫入期間已被其他操作更新，這次修改未套用。請重新載入後再試。');
    throw error;
  }
}
async function updateItem(env: Env, id: string, item: Item, origin: string): Promise<Response> {
  ...
  await createAtomicCommit(env, remote, [
    { path: remote.works[workIndex].path, content: jsonFile(payload) },
    { path: 'public/data/version.json', content: jsonFile({ version: nextVersion }) }
  ], `fix: update item ${id}`);
...
async function deleteItem(env: Env, id: string, origin: string): Promise<Response> {
  ...
  if (workIndex < 0) return fail('WORK_NOT_FOUND', '找不到 Item 所屬作品。', 404, origin);
  const payload = remote.works[workIndex].payload;
  if (!payload.items.some(entry => entry.id === id)) return fail('ITEM_NOT_FOUND', '找不到要刪除的收藏。', 404, origin);
...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    try {
      ...
      if (request.method === 'GET' && url.pathname === '/api/auth/status') return ok({ authenticated: authorized(request, env) }, origin);
      ...
      if (match && request.method === 'PUT') { const body = await request.json() as { item?: unknown }; validateItem(body.item); return await updateItem(env, decodeURIComponent(match[1]), body.item, origin); }
