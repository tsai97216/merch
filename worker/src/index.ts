...
async function githubContents<T = GitHubContentEntry | GitHubContentEntry[]>(env: Env, path: string): Promise<T> {
  return githubJson<T>(env, `/repos/${env.GITHUB_REPO}/contents/${path}?ref=${encodeURIComponent(env.GITHUB_BRANCH)}`);
}
...
async function loadRemoteTarget(env: Env, id: string): Promise<RemoteState> {
  ...
  const readJson = async (path: string) => {
    const entry = await githubContents<GitHubContentEntry>(env, path);
    if (entry.type !== 'file' || !entry.content || !entry.sha) throw new Error(`找不到資料檔：${path}`);
    const content = entry.encoding === 'base64' ? decodeText(entry.content) : entry.content;
    files.set(path, { content, sha: entry.sha }); paths.add(path); return content;
  };
  const indexPath = 'data/works.json';
  const index = JSON.parse(await readJson(indexPath)) as WorkIndex;
  const versionPath = 'public/data/version.json';
  const version = (JSON.parse(await readJson(versionPath)) as { version?: string }).version;
  if (index.schemaVersion !== 2 || !Array.isArray(index.works) || !version) throw new Error('新資料格式無效。');
  const work = index.works.find(entry => id.startsWith(entry.code));
  if (!work) throw new Error(`找不到 Item 所屬作品：${id}`);
  const workRoot = work.path.replace(/\/$/, '');
  const categoryDirs = await githubContents<GitHubContentEntry[]>(env, workRoot);
  if (!Array.isArray(categoryDirs)) throw new Error(`作品目錄格式無效：${workRoot}`);
  ...
  for (const dir of categoryDirs.filter(entry => entry.type === 'dir' && typeof entry.name === 'string' && CATEGORY_RE.test(entry.name))) {
    const categoryPath = `${workRoot}/${dir.name}/index.json`;
    let category: CategoryIndex;
    try { category = JSON.parse(await readJson(categoryPath)) as CategoryIndex; } catch { continue; }
    ...
    const summary = category.items.find(entry => entry.id === id);
    if (!summary) continue;
    const itemPath = summary.path;
    const item = JSON.parse(await readJson(itemPath)) as Item;
    if (item.id !== id || item.workId !== work.id || item.category !== category.category) throw new Error(`Item 索引不一致：${id}`);
    if (!Array.isArray(item.images)) item.images = [];
    items.set(id, { item, path: itemPath, categoryPath, workId: work.id });
    const imageDir = `data/${work.id}/${category.category}/${id}/images`;
    let imageEntries: GitHubContentEntry[] = [];
    try { imageEntries = await githubContents<GitHubContentEntry[]>(env, imageDir); } catch { imageEntries = []; }
    for (const image of imageEntries) {
      if (image.type === 'file' && typeof image.path === 'string' && typeof image.sha === 'string') { paths.add(image.path); blobs.set(image.path, image.sha); }
    }
  }
  const existing = items.get(id);
  if (!existing) throw new Error(`找不到 Item：${id}`);
...
async function getAsset(env: Env, path: string, origin: string): Promise<Response> {
  const remote = await loadRemoteTarget(env, id); const parts = assetParts(path); if (!parts) return fail('INVALID_ASSET_PATH', '圖片路徑格式無效。', 400, origin);
  const entry = remote.items.get(parts.itemId); if (!entry || entry.workId !== parts.workId || entry.item.category !== parts.category) return fail('ITEM_PATH_MISMATCH', '圖片路徑與 Item 所屬位置不一致。', 400, origin);
  if (!remote.paths.has(path)) return fail('ASSET_NOT_FOUND', '找不到圖片。', 404, origin);
  const blob = await githubJson<GitHubBlob>(env, `/repos/${env.GITHUB_REPO}/contents/${path}?ref=${encodeURIComponent(env.GITHUB_BRANCH)}`);
  const content = blob.content.replace(/\s/g, '');
...
async function putAsset(env: Env, asset: AssetRequest, origin: string): Promise<Response> {
  const remote = await loadRemoteTarget(env, id); const parts = assetParts(asset.path); if (!parts) return fail('INVALID_ASSET_PATH', '圖片路徑格式無效。', 400, origin);
  const entry = remote.items.get(parts.itemId); if (!entry) return fail('ITEM_NOT_FOUND', '找不到圖片所屬的收藏。', 404, origin);
  if (entry.workId !== parts.workId || entry.item.category !== parts.category) return fail('ITEM_PATH_MISMATCH', '圖片路徑與 Item 所屬位置不一致。', 400, origin);
  const exists = remote.paths.has(asset.path); const item = normalizeItemForStorage(entry.item); const nextImages = [...item.images]; if (!nextImages.some(image => image.file === parts.file)) nextImages.push(assetMetadata(item, parts.file)); item.images = nextImages;
  const writes: WriteEntry[] = [{ path: asset.path, content: asset.content, encoding: 'base64' }, { path: entry.path, content: jsonFile(item) }];
