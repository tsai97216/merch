...
type GitHubContentEntry = { type?: string; path?: string; name?: string; sha?: string; content?: string; encoding?: string };
async function githubContents<T = GitHubContentEntry | GitHubContentEntry[]>(env: Env, path: string): Promise<T> { return githubJson<T>(env, `/repos/${env.GITHUB_REPO}/contents/${path}?ref=${encodeURIComponent(env.GITHUB_BRANCH)}`); }
...
async function loadRemoteTarget(env: Env, id: string): Promise<RemoteState> {
  ...
  const files = new Map<string, { content: string; sha: string }>(); const blobs = new Map<string, string>(); const paths = new Set<string>();
  const readJson = async (path: string) => { const entry = await githubContents<GitHubContentEntry>(env, path); if (entry.type !== 'file' || !entry.content || !entry.sha) throw new Error(`找不到資料檔：${path}`); const content = entry.encoding === 'base64' ? decodeText(entry.content) : entry.content; files.set(path, { content, sha: entry.sha }); paths.add(path); return content; };
  const index = JSON.parse(await readJson('data/works.json')) as WorkIndex; const version = (JSON.parse(await readJson('public/data/version.json')) as { version?: string }).version;
  if (index.schemaVersion !== 2 || !Array.isArray(index.works) || !version) throw new Error('新資料格式無效。');
  const work = index.works.find(entry => id.startsWith(entry.code)); if (!work) throw new Error(`找不到 Item 所屬作品：${id}`);
  const workRoot = work.path.replace(/\/$/, ''); const categoryDirs = await githubContents<GitHubContentEntry[]>(env, workRoot); if (!Array.isArray(categoryDirs)) throw new Error(`作品目錄格式無效：${workRoot}`);
  const categories = new Map<string, CategoryIndex>(); const items = new Map<string, { item: Item; path: string; categoryPath: string; workId: string }>();
  for (const dir of categoryDirs.filter(entry => entry.type === 'dir' && typeof entry.name === 'string' && CATEGORY_RE.test(entry.name))) {
    const categoryPath = `${workRoot}/${dir.name}/index.json`; let category: CategoryIndex; try { category = JSON.parse(await readJson(categoryPath)) as CategoryIndex; } catch { continue; }
    if (category.schemaVersion !== 1 || category.workId !== work.id || category.category !== dir.name || !Array.isArray(category.items)) throw new Error(`類型索引格式無效：${categoryPath}`);
    categories.set(categoryPath, category); const summary = category.items.find(entry => entry.id === id); if (!summary) continue;
    const item = JSON.parse(await readJson(summary.path)) as Item; if (item.id !== id || item.workId !== work.id || item.category !== category.category) throw new Error(`Item 索引不一致：${id}`); if (!Array.isArray(item.images)) item.images = [];
    items.set(id, { item, path: summary.path, categoryPath, workId: work.id }); const imageDir = `data/${work.id}/${category.category}/${id}/images`; let imageEntries: GitHubContentEntry[] = []; try { imageEntries = await githubContents<GitHubContentEntry[]>(env, imageDir); } catch { imageEntries = []; }
    for (const image of imageEntries) if (image.type === 'file' && typeof image.path === 'string' && typeof image.sha === 'string') { paths.add(image.path); blobs.set(image.path, image.sha); }
  }
  if (!items.has(id)) throw new Error(`找不到 Item：${id}`); return { index, version, headSha, baseTreeSha: head.tree.sha, files, blobs, categories, items, paths };
}
...
async function updateItem(env: Env, id: string, input: unknown, origin: string): Promise<Response> {
  ...
  const remote = await loadRemoteTarget(env, id); const work = remote.index.works.find(entry => entry.id === item.workId); if (!work) return fail('WORK_NOT_FOUND', '找不到 Item 所屬作品。', 404, origin);
...
async function deleteItem(env: Env, id: string, origin: string): Promise<Response> {
  const remote = await loadRemoteTarget(env, id); const existing = remote.items.get(id); if (!existing) return fail('ITEM_NOT_FOUND', '找不到要刪除的收藏。', 404, origin);
