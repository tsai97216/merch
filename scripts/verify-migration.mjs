...
for (const work of newIndex.works) {
  ...
  for (const categoryDir of await fs.readdir(workRoot, { withFileTypes: true })) {
    ...
    const categoryIndex = await readJson(path.join(workRoot, categoryDir.name, 'index.json'));
...
for (const id of legacyIds) {
  ...
  const files = (await fs.readdir(imageDir, { withFileTypes: true }).catch(() => [])).filter(entry => entry.isFile()).map(entry => entry.name).sort();
