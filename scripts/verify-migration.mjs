...
function normalizeLegacy(raw, workId) {
  ...
  const images = Array.isArray(raw.images) ? raw.images.map((image) => {
    ...
    return {
      id: String(source.id ?? path.parse(file).name),
      file,
      ...(typeof source.alt === 'string' ? { alt: source.alt } : {}),
      ...(source.isCover === true ? { isCover: true } : {})
    };
  }) : [];
  return {
    id,
    ...
    purchase: {
      ...(typeof purchase.price === 'number' && Number.isFinite(purchase.price) ? { price: purchase.price } : {}),
    ...
    arrival: {
      ...(typeof release.expectedDate === 'string' ? { expectedDate: release.expectedDate || null } : {}),
      ...(typeof release.receivedDate === 'string' ? { receivedDate: release.receivedDate || null } : {})
    },
    afterSales: {
...
async function findSourceImage(sourcePath, file, workId, itemId) {
  const candidates = [];
  if (sourcePath) {
    const relative = sourcePath.replace(/^\//, '');
    candidates.push(relative.startsWith('data/') ? path.join(root, 'public', relative) : path.resolve(root, relative));
  }
  candidates.push(path.join(sourceRoot, workId, 'images', itemId, file));
  candidates.push(path.join(sourceRoot, workId, 'images', file));
...
if (!Array.isArray(legacyIndex.works) || newIndex.schemaVersion !== 2 || !Array.isArray(newIndex.works)) fail('migration 比對索引格式無效');
if (legacyIndex.works.length !== newIndex.works.length) fail(`作品數量不一致：legacy=${legacyIndex.works.length} new=${newIndex.works.length}`);

const legacyItems = new Map();
const legacyImageSources = new Map();
for (const work of legacyIndex.works) {
  ...
  for (const raw of payload.items) {
    ...
    legacyImageSources.set(normalized.id, Array.isArray(raw.images) ? raw.images.map(image => ({
      file: String(image?.file ?? (typeof image?.path === 'string' ? path.basename(image.path) : '')),
...
for (const work of newIndex.works) {
  ...
  if (!(await exists(workRoot))) fail(`找不到作品目錄：${work.id}`);
  for (const categoryDir of await fs.readdir(workRoot, { withFileTypes: true })) {
    if (!categoryDir.isDirectory()) continue;
    const categoryIndexPath = path.join(workRoot, categoryDir.name, 'index.json');
    if (!(await exists(categoryIndexPath))) continue;
    const categoryIndex = await readJson(categoryIndexPath);
    if (!Array.isArray(categoryIndex.items)) fail(`new category items 無效：${work.id}/${categoryDir.name}`);
    for (const entry of categoryIndex.items) {
      const relativeItemPath = String(entry.path).replace(/^data\//, '');
      const item = await readJson(path.join(newRoot, relativeItemPath));
...
for (const id of legacyIds) {
  ...
  imageCountLegacy += beforeImages.length;
  imageCountNew += afterImages.length;
  if (JSON.stringify(beforeImages) !== JSON.stringify(afterImages)) fail(`圖片 metadata 集合不一致：${id}`);

  const imageDir = path.join(newRoot, before.workId, before.category, id, 'images');
  const files = (await fs.readdir(imageDir, { withFileTypes: true }).catch(() => []))
    .filter(entry => entry.isFile())
    .map(entry => entry.name)
    .sort();
  const expectedFiles = beforeImages.map(image => image.file).sort();
  if (JSON.stringify(files) !== JSON.stringify(expectedFiles)) fail(`圖片檔案集合不一致：${id}`);

  const rawSources = legacyImageSources.get(id) || [];
  for (const image of beforeImages) {
    const sourceMeta = rawSources.find(candidate => candidate.file === image.file);
    const source = await findSourceImage(sourceMeta?.path || '', image.file, before.workId, id);
    if (!source) fail(`找不到 legacy 圖片：${id}/${image.file}`);
    const migrated = path.join(imageDir, image.file);
    const [sourceSha, migratedSha] = await Promise.all([sha256(source), sha256(migrated)]);
    if (sourceSha !== migratedSha) fail(`圖片內容不一致：${id}/${image.file}`);
  }
}
if (legacyItems.size !== newItems.size) fail(`Item 數量不一致：legacy=${legacyItems.size} new=${newItems.size}`);
if (quantityLegacy !== quantityNew) fail(`quantity 總和不一致：legacy=${quantityLegacy} new=${quantityNew}`);
if (imageCountLegacy !== imageCountNew) fail(`圖片總數不一致：legacy=${imageCountLegacy} new=${imageCountNew}`);
console.log(`Migration equivalence passed: ${newIndex.works.length} works, ${newItems.size} items, quantity ${quantityNew}, ${imageCountNew} images.`);
