...
const legacyFiles = (await fs.readdir(dataRoot, { withFileTypes: true }))
  .filter(entry => entry.isFile() && entry.name.endsWith('.json') && !['works.json', 'categories.json', 'version.json'].includes(entry.name));
if (legacyFiles.length) fail(`發現舊作品 JSON：${legacyFiles.map(entry => entry.name).join(', ')}`);

console.log(`Data layout verification passed: ${index.works.length} works, ${itemCount} item records, ${physicalQuantity} physical items, ${imageCount} images.`);
