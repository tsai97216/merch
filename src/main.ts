...
function itemCard(item: Item, compact = false): string {
  ...
  const totalValue = unitPrice == null ? undefined : itemValue(item);
  return `<article class="item-card ${compact ? 'compact':''}" data-item-id="${escapeHtml(item.id)}"><div class="item-media">${src ? `<img src="${escapeHtml(src)}" alt="${escapeHtml(image?.alt || item.title)}" loading="lazy" onerror="this.style.display='none';this.parentElement.classList.add('image-fallback')">` : '<span class="image-placeholder">NO IMAGE</span>'}</div><div class="item-body"><div class="item-top"><span class="badge">${escapeHtml(statusText(item.status))}</span><span class="muted">${escapeHtml(item.category || '未分類')}</span></div><h3>${escapeHtml(item.title)}</h3><p class="item-work">${escapeHtml(item.workName || '')}${item.characters?.length ? ` · ${escapeHtml(item.characters.join('、'))}` : ''}</p><div class="item-bottom"><strong>${money(unitPrice)}${quantity > 1 ? ` × ${quantity} = ${money(totalValue)}` : ''}</strong><span>${escapeHtml(item.manufacturer || '')}</span></div></div></article>`;
}

function renderHome(store: MerchStore) {
  const { items, works } = store.snapshot;
  const total = items.reduce((sum, item) => sum + quantityOf(item), 0);
  const received = items.filter((i) => i.status === 'received').reduce((sum, item) => sum + quantityOf(item), 0);
  const preorder = items.filter((i) => i.status === 'preorder').reduce((sum, item) => sum + quantityOf(item), 0);
  const pending = items.filter((i) => i.status === 'pending' || i.status === 'preorder').reduce((sum, item) => sum + quantityOf(item), 0);
  const spending = items.reduce((sum, i) => sum + itemValue(i), 0);
  const now = new Date();
  const month = items.filter((i) => { const d = new Date(i.purchase?.date || ''); return d.getFullYear() === now.getFullYear() && d.getMonth() === now.getMonth(); }).reduce((s, i) => s + itemValue(i), 0);
...
function renderStatistics(store: MerchStore) {
  const { items, works } = store.snapshot;
  const total = items.reduce((sum, item) => sum + quantityOf(item), 0);
  const pending = items.filter((i) => i.status !== 'received').reduce((sum, item) => sum + quantityOf(item), 0);
  const spending = items.reduce((s, i) => s + itemValue(i), 0);
...
function renderDetail(store: MerchStore, id: string) {
  ...
  setDetailText(modal, 'item-detail-title', item.title || '收藏詳細資訊'); setDetailText(modal, 'item-detail-subtitle', `${item.workName || item.series || '—'} · ${characters}`); setDetailText(modal, 'detail-work', item.workName || item.series); setDetailText(modal, 'detail-character', characters); setDetailText(modal, 'detail-type', item.category); setDetailText(modal, 'detail-manufacturer', item.manufacturer); setDetailText(modal, 'detail-price', money(item.purchase?.price)); setDetailText(modal, 'detail-quantity', quantityOf(item)); setDetailText(modal, 'detail-total-price', item.purchase?.price == null ? undefined : money(itemValue(item))); setDetailText(modal, 'detail-platform', item.purchase?.platform); setDetailText(modal, 'detail-purchase-date', dateText(item.purchase?.date)); setDetailText(modal, 'detail-release-date', dateText(releaseDate)); setDetailText(modal, 'detail-received-date', dateText(item.release?.receivedDate)); setDetailText(modal, 'detail-shipping', shipping); setDetailText(modal, 'detail-after-sales', afterSales); setDetailText(modal, 'detail-description', item.description); setDetailText(modal, 'detail-notes', item.notes); setDetailText(modal, 'detail-created', dateText(item.createdAt)); setDetailText(modal, 'detail-updated', dateText(item.updatedAt));
