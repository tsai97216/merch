...
let detailUnderlyingRoute = 'collection';
...
function ensureDetailModal() {
  ...
  modal.hidden = true;
  modal.innerHTML = `<div class="item-detail-backdrop" data-detail-close></div>
    <section class="item-detail-dialog" role="dialog" aria-modal="true" aria-labelledby="item-detail-title">
      ...
      <div class="item-detail-header">
        ...
        <div class="item-detail-heading"><span class="eyebrow">ITEM DETAIL</span><h2 id="item-detail-title"></h2><p id="item-detail-subtitle"></p><div id="item-detail-status"></div></div>
      </div>
  ...
  modal.querySelectorAll('[data-detail-close]').forEach((node) => node.addEventListener('click', closeDetailModal));
  document.addEventListener('keydown', (event) => { if (event.key === 'Escape' && detailModal && !detailModal.hidden) closeDetailModal(); });
  detailModal = modal;
  return modal;
}

function setDetailText(modal: HTMLElement, id: string, value: unknown) {
  const node = modal.querySelector<HTMLElement>(`#${id}`);
  if (node) node.textContent = value == null || value === '' ? '—' : String(value);
}

function closeDetailModal() {
  if (detailModal) { detailModal.hidden = true; document.body.classList.remove('detail-modal-open'); }
  const target = `#/${detailUnderlyingRoute}`;
...
function renderDetail(store: MerchStore, id: string) {
  ...
  if (!item) {
    const dialog = $('.item-detail-dialog', modal);
    if (dialog) dialog.innerHTML = `<button type="button" class="item-detail-close" aria-label="關閉" data-detail-close><i class="fa-solid fa-xmark"></i></button><div class="item-detail-error"><span class="eyebrow">404 / ITEM</span><h2>找不到收藏</h2><p>這個收藏不存在或已被移除。</p></div>`;
    dialog?.querySelector('[data-detail-close]')?.addEventListener('click', closeDetailModal);
    modal.hidden = false; document.body.classList.add('detail-modal-open'); return;
  }
  ...
  setDetailText(modal, 'item-detail-title', item.title || '收藏詳細資訊');
  setDetailText(modal, 'item-detail-subtitle', `${item.workName || item.series || '—'} · ${characters}`);
  setDetailText(modal, 'detail-work', item.workName || item.series);
  setDetailText(modal, 'detail-character', characters);
  setDetailText(modal, 'detail-type', item.category);
  setDetailText(modal, 'detail-manufacturer', item.manufacturer);
  setDetailText(modal, 'detail-price', money(item.purchase?.price));
  setDetailText(modal, 'detail-platform', item.purchase?.platform);
  setDetailText(modal, 'detail-purchase-date', dateText(item.purchase?.date));
  setDetailText(modal, 'detail-release-date', dateText(releaseDate));
  setDetailText(modal, 'detail-received-date', dateText(item.release?.receivedDate));
  setDetailText(modal, 'detail-shipping', shipping);
  setDetailText(modal, 'detail-tracking', item.shipping?.trackingNumber);
  setDetailText(modal, 'detail-after-sales', afterSales);
  setDetailText(modal, 'detail-description', item.description);
  setDetailText(modal, 'detail-notes', item.notes);
  setDetailText(modal, 'detail-created', dateText(item.createdAt));
  setDetailText(modal, 'detail-updated', dateText(item.updatedAt));
  const status = $('#item-detail-status', modal);
  if (status) status.innerHTML = `<span class="badge">${escapeHtml(statusText(item.status))}</span>`;
  const image = $('#item-detail-image', modal);
  const cover = item.images?.find((entry) => entry.isCover) || item.images?.[0];
  const imageSrc = cover?.url || cover?.path || '';
  if (image) image.innerHTML = imageSrc ? `<img src="${escapeHtml(imageSrc)}" alt="${escapeHtml(cover?.alt || item.title || '')}" onerror="this.style.display='none';this.parentElement.classList.add('image-fallback')">` : '<span class="image-placeholder">IMAGE</span>';
  modal.hidden = false;
...
async function boot() {
  ...
  const router = createRouter({ onNavigate: (route) => {
    if (route.name === 'item') {
      detailUnderlyingRoute = visiblePage || 'collection';
      $$('.page').forEach((page) => { const isBase = page.getAttribute('data-page') === detailUnderlyingRoute; page.toggleAttribute('hidden', !isBase); page.classList.toggle('is-active', isBase); });
      $$('.nav a').forEach((a) => a.classList.toggle('is-active', a.dataset.route === detailUnderlyingRoute));
      renderDetail(storeRef!, route.id);
      return;
    }
    if (route.name === 'not-found') {
      $$('.page').forEach((page) => { const active = page.getAttribute('data-page') === '404'; page.toggleAttribute('hidden', !active); page.classList.toggle('is-active', active); });
      if (detailModal) { detailModal.hidden = true; document.body.classList.remove('detail-modal-open'); }
      return;
    }
    visiblePage = route.name;
    if (detailModal) { detailModal.hidden = true; document.body.classList.remove('detail-modal-open'); }
    $$('.page').forEach((page) => { const active = page.getAttribute('data-page') === route.name; page.toggleAttribute('hidden', !active); page.classList.toggle('is-active', active); });
    $$('.nav a').forEach((a) => a.classList.toggle('is-active', a.dataset.route === route.name));
    const recentSection = $('#recent-section');
    if (recentSection) recentSection.toggleAttribute('hidden', route.name !== 'home');
  }});
  ...
  try {
    ...
    const route = router.current(); if (route.name === 'item') { detailUnderlyingRoute = 'collection'; renderDetail(store, route.id); }
  } catch (error) {
    console.error(error);
