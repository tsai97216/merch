...
function aggregate(items: Item[]): Chart[] {
  ...
  monthlyEntries.forEach(([key, row], index) => {
    ...
    monthlyDetails.push({ label: monthLabel(key), quantity: row.quantity, spend: row.spend, share: totalSpend ? row.spend / totalSpend * 100 : 0, extra: `${formatMoney(cumulative)} 累計 · ${delta}` });
  });
  ...
  return [
    ...
    },
    {
      ...
      rows:makeRows(workCountEntries, 'quantity'), details:detail(workCountEntries, totalQuantity, (row) => `消費 ${formatMoney(row.spend)}`), format:'count',
    ...
    },
    {
      ...
      rows:makeRows(categoryEntries, 'quantity'), details:detail(categoryEntries, totalQuantity, (row) => `消費 ${formatMoney(row.spend)}`), format:'count',
...
async function boot(): Promise<void> { const host=qs<HTMLElement>('#statistics-charts'); if(!host)return; try { const store=await loadStore(); render(aggregate(store.snapshot.items)); } catch { host.innerHTML='<div class="empty-state wide">統計資料載入失敗</div>'; } }
void boot();
