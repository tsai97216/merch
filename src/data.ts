...
async function loadJson<T>(path: string, validate: (value: unknown) => asserts value is T): Promise<T> {
  const response = await fetch(dataUrl(path), { headers: { Accept: 'application/json' } });
...
export function loadWorks(): Promise<WorksFile> {
  return loadJson('data/works.json', validateWorksFile);
}

export function loadVersion(): Promise<VersionInfo> {
  return loadJson('data/version.json', validateVersion);
}

export function loadWorkData(path: string): Promise<WorkData> {
  return loadJson(path, validateWorkData);
}
