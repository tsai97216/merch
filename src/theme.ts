...
function applyTheme(theme: Theme, persist = true): void {
  ...
  if (persist) {
    try { if (theme === 'system') localStorage.removeItem(STORAGE_KEY); else localStorage.setItem(STORAGE_KEY, theme); } catch { /* storage may be unavailable */ }
  }
}
function init(): void {
  ...
  document.addEventListener('click', (event) => {
    const button = (event.target as Element | null)?.closest<HTMLButtonElement>('[data-theme-choice]');
    const theme = button?.dataset.themeChoice;
    if (theme === 'system' || theme === 'dark' || theme === 'light') applyTheme(theme);
  });
  const media = window.matchMedia?.('(prefers-color-scheme: dark)');
  media?.addEventListener('change', () => { if (getStoredTheme() === 'system') applyTheme('system', false); });
