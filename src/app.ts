...
function closeImageViewer(): void { if (!imageViewer) return; imageViewer.remove(); imageViewer = null; document.body.classList.remove('modal-open'); imageViewerPreviousFocus?.focus(); imageViewerPreviousFocus = null; }
function openImageViewer(url: string, alt: string): void {
  if (!url) return; closeImageViewer(); imageViewerPreviousFocus = document.activeElement instanceof HTMLElement ? document.activeElement : null;
  const overlay = document.createElement('div'); overlay.className = 'image-modal'; overlay.setAttribute('role', 'dialog'); overlay.setAttribute('aria-modal', 'true'); overlay.setAttribute('aria-label', '圖片檢視器');
  const panel = document.createElement('div'); panel.className = 'image-modal-panel'; const close = document.createElement('button'); close.className = 'button image-modal-close'; close.type = 'button'; close.dataset.action = 'close-image'; close.setAttribute('aria-label', '關閉圖片檢視器'); close.textContent = '關閉';
  const image = document.createElement('img'); image.src = url; image.alt = alt || '收藏圖片'; image.addEventListener('error', () => image.replaceWith(Object.assign(document.createElement('p'), { className: 'image-modal-error', textContent: '圖片載入失敗。' })), { once: true });
  panel.append(close, image); overlay.append(panel); overlay.addEventListener('click', (event) => { if (event.target === overlay) closeImageViewer(); }); document.body.append(overlay); imageViewer = overlay; document.body.classList.add('modal-open'); close.focus();
}
function render(): void {
  captureFocus(); const state = store.getState();
  if (state.loading) { appMount.replaceChildren(loadingScreen()); document.documentElement.dataset.chiMerchBooted = 'true'; return; }
  if (state.error) { appMount.replaceChildren(errorScreen(state.error)); document.documentElement.dataset.chiMerchBooted = 'true'; return; }
...
appMount.addEventListener('click', (event) => {
  ...
  switch (target.dataset.action) { case 'open-item': { const id = target.dataset.itemId; if (id) navigate({ name: 'item', id }); break; } case 'view-card': store.setCollectionUI({ viewMode: 'card' }); break; case 'view-list': store.setCollectionUI({ viewMode: 'list' }); break; case 'back-collection': navigate({ name: 'collection' }); break; case 'retry': void store.load(); break; case 'view-image': openImageViewer(target.dataset.imageUrl ?? '', target.dataset.imageAlt ?? ''); break; case 'close-image': closeImageViewer(); break; }
});
