...
export class MerchStore {
  ...
  replaceData(works: Work[], version: string, shipping: StoreState['shipping'] = this.state.shipping): void { const normalizedWorks = works.map(work => ({ ...work, items: work.items.map((item, index) => normalizeStoreItem({ ...item, workId: work.id, workName: work.name }, `${work.name}.items[${index}]`)) })); const items = normalizedWorks.flatMap(work => work.items.map(item => ({ ...item, workId: work.id, workName: work.name }))); this.state = cloneAndFreeze({ ...this.state, works: normalizedWorks, items, version, shipping, loading: false, error: null }); this.emit(); }
  private applyRemote(data: { works: Work[]; version: string; shipping: StoreState['shipping'] }): void { this.replaceData(data.works, data.version, data.shipping); }
