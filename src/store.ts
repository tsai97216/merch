...
function validateWorksIndex(value: unknown): WorksIndex {
  ...
  works.forEach((work) => { if (workCodes.has(work.code)) throw dataError(`works.json 存在重複作品代碼：${work.code}`); workCodes.add(work.code); });
  return { schemaVersion: 1, works };
}
function normalizeQuantity(value: unknown, label: string): number {
  if (value === undefined) return 1;
  if (typeof value !== 'number' || !Number.isInteger(value) || value < 1) throw dataError(`${label}.quantity 必須是大於等於 1 的整數`);
  return value;
}
function validateItem(value: unknown, label: string, work: WorksIndexEntry): Item {
  ...
  if (value.characters !== undefined && (!Array.isArray(value.characters) || value.characters.some((x) => typeof x !== 'string'))) throw dataError(`${label}.characters 格式無效`);
  if (value.images !== undefined && (!Array.isArray(value.images) || value.images.some((x) => !isRecord(x) || typeof x.id !== 'string'))) throw dataError(`${label}.images 格式無效`);
...
function validateWorkPayload(value: unknown, label: string, work: WorksIndexEntry): WorkPayload {
  ...
  items.forEach((item) => {
    ...
    if (parsed) { const groupKey = `${parsed.workCode}${parsed.categoryCode}${parsed.sequence}`; if (groups.has(groupKey)) throw dataError(`${label} 存在重複永久流水號：${item.id}`); groups.add(groupKey); }
  });
...
function deepFreeze<T>(value: T): T {
  if (value && typeof value === 'object' && !Object.isFrozen(value)) { Reflect.ownKeys(value as object).forEach((key) => { const child = (value as Record<PropertyKey, unknown>)[key]; if (child && typeof child === 'object') deepFreeze(child); }); Object.freeze(value); }
  return value;
}
...
function readSavedUi(): UiState {
  try {
    const raw = localStorage.getItem('merch-ui'); if (!raw) return { ...defaultUiState }; const parsed = JSON.parse(raw) as Partial<UiState>;
    return { collectionView: parsed.collectionView === 'list' ? 'list' : 'cards', collectionQuery: typeof parsed.collectionQuery === 'string' ? parsed.collectionQuery : '', collectionStatus: typeof parsed.collectionStatus === 'string' ? parsed.collectionStatus : 'all', collectionWork: typeof parsed.collectionWork === 'string' ? parsed.collectionWork : 'all', collectionCategory: typeof parsed.collectionCategory === 'string' ? parsed.collectionCategory : 'all', collectionCharacter: typeof parsed.collectionCharacter === 'string' ? parsed.collectionCharacter : 'all', collectionManufacturer: typeof parsed.collectionManufacturer === 'string' ? parsed.collectionManufacturer : 'all', collectionSort: parsed.collectionSort === 'title' || parsed.collectionSort === 'price' ? parsed.collectionSort : 'created' };
...
export function loadStore(): Promise<MerchStore> {
  ...
  sharedStorePromise = (async () => {
    ...
    try {
      ...
      works.forEach((work) => work.items.forEach((item) => { if (allIds.has(item.id)) throw dataError(`所有作品存在重複 Item ID：${item.id}`); allIds.add(item.id); }));
      ...
      const appError = toAppError(error, '資料載入失敗'); store.setError(appError.message); sharedStorePromise = null; throw Object.assign(new Error(appError.message), { store, code: appError.code, cause: appError.cause });
    }
  })();
