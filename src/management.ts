...
function render() {
  ...
  root.innerHTML = `<div class="management-toolbar"><label class="field compact"><span>編輯收藏</span><select id="management-item-select"><option value="">選擇收藏</option>${items.map(x => `<option value="${escapeHtml(x.id)}">${escapeHtml(x.id)} · ${escapeHtml(x.title)}</option>`).join('')}</select></label><span class="management-readonly">目前只驗證草稿，不直接寫入 GitHub。</span></div>
  ...
  if (item) fill(item);
  const select = qs<HTMLSelectElement>('#management-item-select'); if (select) select.value = selectedId;
  select?.addEventListener('change', () => { const next = items.find(x => x.id === select.value); if (next) { fill(next); render(); } });
