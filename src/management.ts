...
async function handleSubmit(event: SubmitEvent): Promise<void> { event.preventDefault(); if (!storeRef || saving || imageSaving) return; const base = selectedItem(); if (!base) return showToast('找不到要操作的收藏。', 'error'); const item = readFormItem(base); const errors = validateItem(item); if (errors.length) return showToast(errors.join('\n'), 'error'); saving = true; render(); showToast('收藏修改同步中，請稍候。', 'info'); try { await storeRef.updateItem(item); selectedId = item.id; showToast('收藏已成功更新。', 'success'); } catch (error) { showToast(error instanceof Error ? error.message : '儲存失敗。', 'error'); } finally { saving = false; render(); } }
