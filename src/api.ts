...
async function request(path: string, init: RequestInit = {}): Promise<unknown> {
  ...
  if (!response.ok || !payload?.ok) { const failure: ApiFailure = { apiCode: payload?.error?.code, status: response.status }; throw dataError(payload?.error?.message || `API 請求失敗（${response.status}）`, failure); }
  return payload.data;
}
