...
function renderPanel(): void {
  const page = document.querySelector<HTMLElement>('[data-page="management"]');
  const heading = page?.querySelector<HTMLElement>('.page-heading');
  if (!page || !heading) return;
  if (panel?.isConnected) return;

  panel = document.createElement('section');
  panel.className = 'panel management-auth-panel';
  panel.setAttribute('aria-labelledby', 'management-auth-title');
  panel.innerHTML = `<div class="management-auth-heading"><div><span class="panel-label">ADMIN ACCESS</span><h2 id="management-auth-title">管理驗證</h2><p>遠端收藏修改需要 Admin Secret。驗證資訊只會暫存在目前瀏覽器工作階段。</p></div><span class="management-auth-status" data-auth-status>未驗證</span></div><div class="management-auth-controls"><label class="field"><span>Admin Secret</span><input id="management-admin-secret" type="password" autocomplete="current-password" spellcheck="false" placeholder="輸入管理驗證密鑰"></label><div class="management-auth-buttons"><button type="button" class="button" data-auth-verify>驗證</button><button type="button" class="button secondary" data-auth-clear>清除</button></div></div><p class="management-auth-message" data-auth-message role="status"></p>`;
  ...
  panel.querySelector<HTMLButtonElement>('[data-auth-clear]')?.addEventListener('click', () => {
    clearAdminSecret();
    updateStatus(false);
    const input = panel?.querySelector<HTMLInputElement>('#management-admin-secret');
...
async function verifyStored(input: HTMLInputElement): Promise<void> {
  try {
    ...
    // A missing or unavailable API is handled by the management write itself.
  }
  clearAdminSecret();
  input.value = '';
  updateStatus(false);
...
function syncRoute(): void {
  const isManagement = location.hash === '#/management';
  if (!isManagement) {
    panel = null;
    return;
  }
