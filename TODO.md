...
- 最新程式修改 commit：`f866e02d159dffdd44ff12794628fd5bd660c456`。
...
- Home 角色資料仍依累計花費計算內部排序依據；多角色商品的金額按唯一角色平均分攤，並補上平手／競賽排名、排序與分攤契約驗證。
- 首頁角色資料的使用者介面已改為「角色偏愛」：不顯示排行文字或名次數字，改用愛心標記與角色名稱表達偏愛。
- 角色偏愛首頁預覽維持 5 個角色；點入後顯示完整角色清單，同樣不顯示名次與花費數字。
- 作品消費排行維持原有的金額排行資訊，不與角色偏愛混用。
...
## 7. Home 角色偏愛
- [x] 改為依角色累計花費計算內部偏愛順序，免費周邊以 NT$0 計算。
- [x] 多角色商品的消費金額按唯一角色平均分攤，避免同一筆消費被重複計算。
- [x] 驗證相同花費顯示相同名次。
- [x] 驗證後續名次依競賽排名規則遞延。
- [x] 驗證排序與顯示邏輯一致。
- [x] 驗證角色資料與作品消費 Panel 內部結構。
- [x] 重新設計首頁角色區塊，不再使用「排行」文字或名次數字。
- [x] 改用愛心標記與角色名稱呈現，預覽維持 5 個角色。
- [x] 點入明細移除名次與花費數字，改為完整角色偏愛清單。
- [x] 重新整理角色偏愛區塊的呈現語意，讓它與作品消費區分為「消費資訊」與「喜愛資訊」兩種不同視覺角色。
...
- [x] 角色資料改以花費計算內部偏愛順序，並保留既有平手、後續名次與多角色金額分攤驗證。
