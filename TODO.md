...
- [ ] 修正一般 Work／Shipping mutation 成功後仍顯示錯誤的前端 response contract：Worker mutation endpoint 回傳 `{ version }`，前端部分方法卻直接以完整 `ApiData` 驗證，導致寫入成功後在 response validation 階段誤報失敗。
