...
- [x] 修正 repo 版本同步：`package.json`、`public/data/version.json` 與 Worker source version 已更新至 `1.109.801`，且版本驗證器現在會三方檢查。
...
- [ ] 正式 Worker 尚需部署 `1.109.801`，並重新驗收新增／替換圖片，確認正式環境不再使用舊 Worker。
- [ ] 新發現：R2 rollback 需要先讀取既有物件，但單次 R2 `get()` 的暫時性失敗目前會直接中止 PUT 並回傳 502；應在不改變 rollback 語意的前提下加入有限重試，再判定 R2 previous read 失敗。
