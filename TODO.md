...
> 只保留尚未完成、待驗證或值得持續追蹤的工作。以下順序先處理實際開發與修正，再集中處理所有需要實機／回歸／發布驗收的工作，避免「修正」與「驗收」交錯。已完成的一次性工作移出；已確立且會長期影響開發的規則放在 `RULES.md`。
...
### 16. Collection 初始資料載入與 fallback 最終驗收
...
### 20. 全站 Responsive：Mobile / Tablet / iPad 最終驗收
