...
- [ ] 第 2 段：檔案架構與舊殘留，盤點 `src/`、`public/`、`scripts/`、API / Worker、CSS、HTML、migration，找出未使用檔案、dead code、舊 route、舊 API 與舊資料格式
  - [ ] **疑似 Legacy / duplicate implementation**：確認 `src/main.ts` 與 `src/management.ts`、`src/statistics.ts`、`src/shipping.ts`、`src/works-management.ts` 等模組的實際 production dependency；若僅為舊實作或測試殘留，再評估清理
