...
### 1. 圖片上傳 API 邊界驗收
- [ ] 正式 Worker 部署 `1.109.813`，確認正式環境已使用最新 Worker。
- [ ] 驗收小圖、接近 8 MB 圖片、替換既有圖片三種情境。
- [ ] 驗證前端單檔 8 MB 上限、Base64 約 33% 膨脹，以及 Worker GitHub mutation 10 MB Base64 content limit 的邊界行為。

### 2. 圖片刪除 R2 一致性
- [ ] 根因：`mirrorDelete()` 先完成 GitHub metadata／檔案刪除，再嘗試刪除 R2；若 R2 delete 失敗，GET 仍優先讀到舊 R2 圖片，造成已刪除圖片繼續可見。
- [ ] 修正刪除流程，使 R2 與 GitHub mutation 具備可 rollback 的一致性順序，並加入對應驗證。
