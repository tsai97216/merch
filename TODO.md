...
- 首頁角色區塊現在只以「角色」作為語意，不顯示排行文字、名次數字或愛心圖示。
- 首頁預覽維持 5 個角色，改為純文字卡片式排列；點入後使用相同視覺語言顯示完整角色清單。
- 作品消費排行維持原有的金額排行資訊，不與角色區塊混用。
...
- [x] 角色區塊移除排行、名次與愛心等額外語意，只保留「角色」。
- [x] 首頁預覽維持 5 個角色，改為無圖示、無數字的純文字卡片。
- [x] 點入明細使用相同的角色卡片視覺，避免明細過長。
- [x] 重新整理角色區塊的視覺層級，使其與作品消費資訊形成清楚區隔。
